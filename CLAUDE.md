...
- **Structure des données**: Les données sont préparées via le dépôt `memoire_pot_growth` 
- **Variables d'intérêt**: 
  ...
  - Positions sur les enjeux politiques (carbone, immigration, etc.)
...
1. **Extraction** : Les données sont téléchargées via le package `ces` R
2. **Nettoyage** : Les scripts dans `data_raw/clean_canada/` nettoient les données pour chaque année
3. **Fusion** : Les données sont fusionnées via `data_raw/04_merge_canada.R`
4. **Analyse** : Les scripts dans `R/` effectuent les analyses statistiques
...
- Étude des positions politiques sur les enjeux clés (immigration, environnement, etc.)
- Identification des groupes à fort potentiel de croissance

## Notes importantes
- Les données sont disponibles dans `data/marts/ces_merged.rds`
